...
public:
                                   ...
                                   ZF_IN zfbool forwardToChildScrollView)
    {
        ...
        const QMetaObject *implClass = ((QWidget *)this->_ZFP_ownerZFUIScrollView->nativeView())->metaObject();
        QWidget *vParent = v->parentWidget();
        if(vParent != NULL && this->_ZFP_isSubclass(vParent->metaObject(), implClass) && !this->_ZFP_isSubclass(v->metaObject(), implClass))
        {
            QMouseEvent *eventTmp = _ZFP_ZFUIScrollViewImpl_sys_Qt_MouseEventClone(
                e, e->type(), QPointF(e->localPos().x() + v->geometry().x(), e->localPos().y() + v->geometry().y()));
            QCoreApplication::instance()->sendEvent(vParent, eventTmp);
            this->mouseEventCleanup(eventTmp);
