...
public:
                  ...
                  ZF_IN zfindex atIndex)
    {
        ...
        {
            if(!layoutParam->classData()->classIsSubclassOf(this->thisView->layoutParamClass()))
            {
                layoutParamNeedRelease = zftrue;
        ...
        ZFUIView *virtualParent = this->virtualParent(this->thisView);
...
void ZFUIView::layoutRequest(void)
{
    if(!d->layoutRequested && _ZFP_ZFUIView_layoutRequestOverrideFlag == 0)
    {
        ...
        {
            toNotify[i]->layoutOnLayoutRequest();
        }
    }
}
