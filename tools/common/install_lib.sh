...
find "$SRC_PATH" -regex ".*/.*\.dll" 2>/dev/null | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>/dev/null
find "$SRC_PATH" -regex ".*/.*\.a" 2>/dev/null | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>/dev/null
find "$SRC_PATH" -regex ".*/.*\.so" 2>/dev/null | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>/dev/null
find "$SRC_PATH" -regex ".*/.*\.dylib" 2>/dev/null | xargs -I{} cp -R {} "$DST_PATH" >/dev/null 2>/dev/null

for line in $(find "$DST_PATH" -type f -name "*.a" 2>/dev/null); do
    OLD_DIR=$(pwd)
    cd "$(dirname "$line")"
    rm "lib$PROJ_NAME.a.1" >/dev/null 2>/dev/null
    ln -s "lib$PROJ_NAME.a" "lib$PROJ_NAME.a.1"
    rm "lib$PROJ_NAME.a.1.0" >/dev/null 2>/dev/null
    ln -s "lib$PROJ_NAME.a" "lib$PROJ_NAME.a.1.0"
    rm "lib$PROJ_NAME.a.1.0.0" >/dev/null 2>/dev/null
...
for line in $(find "$DST_PATH" -type f -name "*.so" 2>/dev/null); do
    OLD_DIR=$(pwd)
    cd "$(dirname "$line")"
    rm "lib$PROJ_NAME.so.1" >/dev/null 2>/dev/null
    ln -s "lib$PROJ_NAME.so" "lib$PROJ_NAME.so.1"
    rm "lib$PROJ_NAME.so.1.0" >/dev/null 2>/dev/null
    ln -s "lib$PROJ_NAME.so" "lib$PROJ_NAME.so.1.0"
    rm "lib$PROJ_NAME.so.1.0.0" >/dev/null 2>/dev/null
...
for line in $(find "$DST_PATH" -type f -name "*.dylib" 2>/dev/null); do
    OLD_DIR=$(pwd)
    cd "$(dirname "$line")"
    rm "lib$PROJ_NAME.1.dylib" >/dev/null 2>/dev/null
    ln -s "lib$PROJ_NAME.dylib" "lib$PROJ_NAME.1.dylib"
    rm "lib$PROJ_NAME.1.0.dylib" >/dev/null 2>/dev/null
    ln -s "lib$PROJ_NAME.dylib" "lib$PROJ_NAME.1.0.dylib"
    rm "lib$PROJ_NAME.1.0.0.dylib" >/dev/null 2>/dev/null
